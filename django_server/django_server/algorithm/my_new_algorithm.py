...
def reformat_distance_matrix(events, dist_matrix, scikit_model, latitude_in, longitude_in, latitude_out, longitude_out):
    ...
    for index, i in enumerate(dist_matrix):
        ...
        minutes_in = get_minutes(scikit_model, latitude_in, longitude_in, events[index]['latitude'], events[index]['longitude'])
        help_array.insert(0, copy.deepcopy(minutes_in))

        minutes_out = get_minutes(scikit_model, events[index]['latitude'], events[index]['longitude'], latitude_out, longitude_out)
...
def get_route(recom_events, date, time_to, time_from, latitude_in, longitude_in, latitude_out, longitude_out):
    ...
    dist_matrix = reformat_distance_matrix(events, dist_matrix, scikit_model, latitude_in, longitude_in, latitude_out, longitude_out)
